...
export default function AdminPics() {
    ...
    const [toggleResetOldPics, setToggleResetOldPics] = useState(false)
    ...
    useEffect(async() => {
        ...
        if (descriptionsPicNames.length > 0) {
            for (let picName of descriptionsPicNames) {
                console.log(addPicsNames.indexOf(picName))
    ...
    const handleRemoveExistingPic = (e) => {
        let picName = e.target.id
        AddPicsService.removePicDBandFB(picName)
        .then(res => {
            console.log(res)
            setToggleResetOldPics(!toggleResetOldPics)
        })
    }
    ...
    return (
        ...
        {addPicsUrls.map((picUrl, idx) => (
            ...
            <label>Tags</label>
            {picsDBInfo['tagslists'][addPicsNames[idx]] ? 
                <>
        ...
        {oldPics.map(picData => (
            ...
            <button id={picData[0]} className="remove-pic-btn" onClick={handleRemoveExistingPic}>X</button>
            </>
        ))}
        </div>
    );
